...
void Class_Steering_Wheel_Chassis::Init(float __Velocity_X_Max, float __Velocity_Y_Max, float __Omega_Max, float __Steer_Power_Ratio)
{
    ...
    Motor_Steer[0].Set_Zero_Position(-1.53f);
    Motor_Steer[1].Set_Zero_Position(2.39f);
    Motor_Steer[2].Set_Zero_Position(-2.29f);
    Motor_Steer[3].Set_Zero_Position(-2.74f);
    #endif

    // 底盘速度xPID, 输出摩擦力
    PID_Velocity_X.Init(120.0f, 0.0f, 0.0f, 0.0f, 150.0f, 1000.0f, 0.002f);

    // 底盘速度yPID, 输出摩擦力
    PID_Velocity_Y.Init(120.0f, 0.0f, 0.0f, 0.0f, 150.0f, 1000.0f, 0.002f);
        ...
        case(Chassis_Control_Type_Drive):
        {
            ...
            if (fabs(Target_Velocity_X) < 0.01 && fabs(Target_Velocity_Y) < 0.01 && fabs(Target_Omega) < 0.01)
            {
                ...
                if (Lock_Flag)
                {
                    ...
                    Motor_Steer[0].Set_Target_Radian( PI / 2.f - THETA );
                    Motor_Steer[1].Set_Target_Radian( THETA - PI / 2.f );
                    Motor_Steer[2].Set_Target_Radian( PI / 2.f - THETA );
                    Motor_Steer[3].Set_Target_Radian( THETA - PI / 2.f );
                    for (int i = 0; i < 4; i++)
                    {
            ...
            True_Vx[0] = True_Vx[1] = Slope_Velocity_X.Get_Out() - cosf((PI/2.f) - THETA) * Target_Omega *  R_DIST * 4.0f;
            True_Vx[2] = True_Vx[3] = Slope_Velocity_X.Get_Out() + cosf((PI/2.f) - THETA) * Target_Omega *  R_DIST * 4.0f;

            True_Vy[0] = True_Vy[3] = Slope_Velocity_Y.Get_Out() + sinf((PI/2.f) - THETA) * Target_Omega *  R_DIST * 4.0f;
            True_Vy[1] = True_Vy[2] = Slope_Velocity_Y.Get_Out() - sinf((PI/2.f) - THETA) * Target_Omega *  R_DIST * 4.0f;
...
float wwx,wwy;
float H7_Offset_X = 0.072f, H7_Offset_Y = 0.158f, Distance_Offset = 0.0f;
...
void Class_Steering_Wheel_Chassis::Stree_Angle_Resolution()
{
    ...
    True_Vx[0] = True_Vx[1] = tmp_tx - cosf((PI/2.f) - THETA) * Target_Omega *  R_DIST;
    True_Vx[2] = True_Vx[3] = tmp_tx + cosf((PI/2.f) - THETA) * Target_Omega *  R_DIST;

    True_Vy[0] = True_Vy[3] = tmp_ty + sinf((PI/2.f) - THETA) * Target_Omega *  R_DIST;
    True_Vy[1] = True_Vy[2] = tmp_ty - sinf((PI/2.f) - THETA) * Target_Omega *  R_DIST;
...
void Class_Steering_Wheel_Chassis::TIM_Calculate_PeriodElapsedCallback(Enum_Sprint_Status __Sprint_Status)
{
    ...
    for (int i = 0; i < 4; i++)         //数据传递处理
    {
        ...
        Power_Management.Motor_Data[i].feedback_omega = Motor_Wheel[i].Get_Now_Omega_Radian() * RAD_TO_RPM * 8.0f;
